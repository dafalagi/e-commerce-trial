...
class GetOrderService extends DefaultService implements ServiceInterface {
    ...
    public function process($dto)
    {
        ...
        $model = Order::orderBy($dto['sort_by'], $dto['sort_type']);
        ...
        if (isset($dto['search_param']) and $dto['search_param'] != null) {
            $model->where('payment_status','ILIKE','%'.$dto['search_param'].'%');
        }
        ...
        if (isset($dto['order_uuid']) and $dto['order_uuid'] != '') {
            $model->where('uuid', $dto['order_uuid']);
            $data = $model->first();
        } else {
        ...
        $this->results['message'] = __('success.order.order.fetched');
